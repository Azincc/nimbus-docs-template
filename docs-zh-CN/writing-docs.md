...
这两项不会出现在首次部署表单，需要自定义图片时再添加。图片 URL 或仓库路径会覆盖对应 JSON 字段；未设置时沿用 JSON，未配置对应字段时使用模板内置的 Nimbus 官方 Logo。原有 `default` 和空白值继续兼容相同规则。变量在 Cloudflare Builds 区域保存后，需要重新构建才会生效。具体见[品牌资源](./site-config.md#品牌资源)。
