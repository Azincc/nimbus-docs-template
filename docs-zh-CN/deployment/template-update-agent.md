...
按用户在当前任务中授予的权限操作。阅读本页本身不授予推送、合并或部署权限。先准备可供审阅的更新结果，再在已授权范围内继续。
...
如果生产分支已包含目标提交、无须更新，直接报告已是最新，不创建空提交或更新请求。
...
本地配置可能与 Cloudflare 不同，请说明本次验证使用的文档源。验证私有文档时，可以使用 Cloudflare 已配置的分支构建，不要求用户把 Token 发到聊天。
...
重试旧构建不会包含新的模板代码。页脚及 `/_build.json` 的 SHA 是文档版本，不能单独证明模板已更新。报告时应区分 GitHub 检查通过、PR 已合并和网站已部署，按实际证据说明进度。
