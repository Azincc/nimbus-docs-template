...
当前模板版本支持以下两个可选的普通变量。不设置时，沿用站点配置中的图片。
...
这两项独立生效，修改 Logo 不会同时改变浏览器图标。保留变量并将值留空，即可恢复使用站点配置中的对应图片。

较早创建的部署副本可能还不支持这两个变量。若修改后重新构建仍无变化，需要由模板维护者先同步新版构建脚本。添加变量不会自动升级部署副本。
...
要恢复模板默认值，删除对应构建变量，保存后重新构建。保留变量但将值留空会产生不同的结果：
